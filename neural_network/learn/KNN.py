...
def classify0(inX,dataSet,labels,k):
    ...
    #------实现classify0()方法的第一种方式------
    #1.距离计算
    dataSetSize = dataSet.shape[0]
    #tile生成和训练样本对应的矩阵，并与训练样本求差
    """
    tile:列-3表示复制的行数，行-1/2表示对inx的重复次数
    ...
    #argsort()是将X中元素从小到大排列，提取其对应的index(索引)，然后输出到Y
    #例如： Y=array([3,0,2,1,4,5]) 则，x[3]=-1最小，所以Y[0]=3,x[5]最大，所以Y[5]=5
    ...
    for i in range(k):
        ...
        #在字典中将该类型加1
        #如字典的get方法
        #如：list.get(k,d) 其中get相当于一条if...else...语句，在参数k在字典中，字典将返回list[k];如果参数k不在字典中则返回参数d，如果k在字典中则返回k对应的value值
        # 1 = {5:2,3:4}
        #print l.get(3,0)返回的值是4
        #print l.get(1,0)返回的值是0
        classCount[voteIlabel] = classCount.get(voteIlabel,0) + 1
    
    #3.排序并返回出现最多的那个类型
    #字典的item()方法，以列表返回可遍历的(键，值)元组数组
    #例如：dict = {'Name':'Zara','Age':'7'} print "Value : %s" % dict.items() Value : [('Age',7),('Name','Zara')]
    #sorted中的第2个参数key=operator.itemgetter(1)这个参数的意思是先比较第几个元素
    # 例如: a=[('b',2),('a',1),('c',0)]  b=sorted(a,key=operator.itemgetter(1)) >>>b=[('c',0),('a',1),('b',2)] 可以看到排序是按照后边的0,1,2进行排序的，而不是a,b,c
    # b=sorted(a,key=operator.itemgetter(0)) >>>b=[('a',1),('b',2),('c',0)] 这次比较的是前边的a,b,c而不是0,1,2
    # b=sorted(a,key=operator.itemgetter(1,0)) >>>b=[('c',0),('a',1),('b',2)] 这个是先比较第2个元素，然后对第一个元素进行排序，形成多级排序。
...
def img2vector(filename):
    """
    将图像数据转换为向量
    :parameters filename: 图片文件 因为我们的输入数据的图片格式是32*32的
    :return :一维矩阵
...
def handwritingClassTest():
    # 1. 导入数据
    hwLabels = []
    trainingFileList = listdir('F:\\class\\neural_network\\digits\\trainingDigits')  # load the training set
    ...
    for i in range(m):
        ...
        trainingMat[i, :] = img2vector('F:\\class\\neural_network\\digits\\trainingDigits/%s' % fileNameStr)

    # 2. 导入测试数据
    testFileList = listdir('F:\\class\\neural_network\\digits\\testDigits')  # iterate through the test set
    errorCount = 0.0
    ...
    for i in range(mTest):
        ...
        fileStr = fileNameStr.split('.')[0]  # take off .txt
        classNumStr = int(fileStr.split('_')[0])
        vectorUnderTest = img2vector('F:\\class\\neural_network\\digits\\testDigits/%s' % fileNameStr)
